	...
	public static void main(String[] args) {
		...
		
		{
			...
			for (int i = 0; i < repeats; i++) {
				...
				TabuSearch<CutsTabuIndividual> ts = new TabuSearch<CutsTabuIndividual>(evaluator,10, 350 );
				CutsTabuIndividual ti = ts.search(init);
				ds.addValue(evaluator.evaluate(ti));
			}
			log.debug("best tabu_mst: " + ds.getMean() + "," + ds.getMin());
		}
		
		{
			...
			log.debug("best tabu_mst wPen: " + ds.getMean() + "," + ds.getMin());
		}
		...
		{
			TabuSearch.rndMoveDiversication = true;
			CutsTabuIndividual.k = Integer.MAX_VALUE;
			Evaluator<CutsTabuIndividual> evaluator = new WSSCutsTabuEvaluator(fDist);
			DescriptiveStatistics ds = new DescriptiveStatistics();
			for (int i = 0; i < repeats; i++) {
				CutsTabuIndividual init = new CutsTabuIndividual(GraphUtils.getMinimumSpanningTree(cm, fDist), numCluster );
			...
			log.debug("best tabu_mst wPen Inv: " + ds.getMean() + "," + ds.getMin());
		}
		...
		
		{
			...
			log.debug("best ga_mst rDist: " + ", "+ ds.getMean() + "," + ds.getMin());
		}
		...
		
		{
			...
			log.debug("best ga_full: " + ", "+ ds.getMean() + "," + ds.getMin());
		}
	}
}
