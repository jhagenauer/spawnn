...
public class TreeIndividual2 extends TreeIndividual {
	...
	public static boolean onlyMutTrees = false, onlyMutCuts = false;
	...
	public TreeIndividual recombine(TreeIndividual mother) {
		...
		if (onlyMutTrees || !onlyMutCuts) {
			...
			nTree = new HashMap<double[], Set<double[]>>();
			for (Entry<double[], Set<double[]>> m : tree.entrySet())
				nTree.put(m.getKey(), new HashSet<double[]>(m.getValue()));
		}

		Map<double[], Set<double[]>> nCuts = new HashMap<double[], Set<double[]>>();
		if (onlyMutCuts || !onlyMutTrees) {
			...
			nCuts = new HashMap<double[],Set<double[]>>();
