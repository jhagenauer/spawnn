...
/* For recombination, an intersectiong is chosen and grown... sux big time
 * 
 */
public class TreeIndividual3 extends TreeIndividual {
	
	public TreeIndividual3(Map<double[], Set<double[]>> cm, Map<double[], Set<double[]>> tree, int numCluster) {
		super(cm,tree,numCluster);
	}

	public TreeIndividual3(Map<double[], Set<double[]>> cm, Map<double[], Set<double[]>> tree, Map<double[], Set<double[]>> cuts) {
		super(cm,tree,cuts);
	}
	...
	public TreeIndividual recombine(TreeIndividual mother) {
		...
		double[] na = new ArrayList<double[]>(init.keySet()).get(r.nextInt(init.keySet().size()));
		double[] nb = new ArrayList<double[]>(init.get(na)).get(r.nextInt(init.get(na).size()));
		nTree.get(na).add(nb);
		nTree.get(nb).add(na);
		...
		return new TreeIndividual3(cm, nTree, nCuts);
	}
}
