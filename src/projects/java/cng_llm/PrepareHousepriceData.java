...
public class PrepareHousepriceData {
	...
	public static void main(String[] args) {
		...
		double totArea = 0;
		List<Geometry> ausGeoms = new ArrayList<Geometry>();
		//for( Geometry g : DataUtils.readSpatialDataFrameFromShapefile( new File("/home/julian/git/spawnn/data/marco/dat4/bez_2008.shp"),false).geoms ) {
		for( Geometry g : DataUtils.readSpatialDataFrameFromShapefile( new File("/home/julian/tmp/east_aust.shp"),false).geoms ) {
				
			int num = 0;
			for( Geometry p : sdf.geoms )
				if( p.within(g) )
					num++;
			if( num > 0 ) {
				ausGeoms.add(g);
		...
		while( samples.size() < 2000) {
			double rn = r.nextDouble()*totArea;
			double curArea = 0;
			double[] d = null;
			for( Geometry g : ausGeoms ) {
				if( curArea <= rn && rn <= curArea+g.getArea() ) {
					List<double[]> candidates = new ArrayList<double[]>();
					for( double[] s : sdf.samples ) {
						Geometry geom = sdf.geoms.get(sdf.samples.indexOf(s));
						if( geom.within( g ) && !samples.contains(s) )
							candidates.add(s);
					}
					if( !candidates.isEmpty() )
						d = candidates.get(r.nextInt(candidates.size()));
					break;
				}
				curArea+=g.getArea();
			}
