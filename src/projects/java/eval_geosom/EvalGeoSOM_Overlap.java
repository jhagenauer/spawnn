...
public class EvalGeoSOM_Overlap {

	private static Logger log = Logger.getLogger(EvalGeoSOM_Overlap.class);
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (double k : new double[]{ 0.3 } ) {
				...
				for( int i = 0; i < 200; i++ ) {
					double[] d = new double[]{r.nextDouble() * 0.4, 0.9 + r.nextDouble() * 0.1 };
					samples.add(d);
					cluster.get(0).add(d);
				}
				for( int i = 0; i < 200; i++ ) {
					double[] d = new double[]{r.nextDouble() * 0.4 + 0.4 + k, 0 + r.nextDouble() * 0.1 };
					samples.add(d);
					cluster.get(1).add(d);
				}
				for( int i = 0; i < 200; i++ ) {
					double[] d = new double[]{r.nextDouble() * 0.4 + 0.8, 0.9 + r.nextDouble() * 0.1 };
					samples.add(d); 
					cluster.get(2).add(d);
				}
