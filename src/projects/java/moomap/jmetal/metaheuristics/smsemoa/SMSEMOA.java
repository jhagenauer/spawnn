...
public class SMSEMOA extends Algorithm {
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		boolean localOpt = (Boolean)getInputParameter("localOpt");
		...
		while (evaluations < maxEvaluations) {				
			...
			if( evaluations % 100 == 0 ) {
				...
				log.info(evaluations+", "+d1+", "+d2+", "+d3 );
			}
						
			if( evaluations % 10000 == 0 ) {
				log.debug(evaluations+" evals, writing...");
				...
				// with opt.
				{
					...
					for( int i = 0; i < population.size(); i++ ) 
						ss.add( localOpt(population.get(i), mutationOperator ) );
					...
					try {
						File dir = new File("output/"+evaluations+"_lo/");
				    	dir.mkdirs();
			...
			// evaluation, could also be interpreted as generations
			evaluations++;
